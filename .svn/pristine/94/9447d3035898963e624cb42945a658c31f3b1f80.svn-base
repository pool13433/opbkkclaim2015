...
public class UiStampPayment extends javax.swing.JInternalFrame {
    ...
    private void loadComboCriteriaYear() {
        ...
        int[] arrYear = NumberUtil.INDEX_YEAR(lengthYear);
                    ...
                    if (selectOption == JOptionPane.YES_OPTION) {
                        ...
                        Thread thread = new Thread(new Runnable() {
                            ...
                            public void run() {
                                 ...
                                 */
                                String desc = " รายงานการจ่ายเงิน " + ConstantVariable.PROCE_TABLE_PAYMENT[combo_payment_type.getSelectedIndex()];
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    if (combo_payment_type.getSelectedIndex() > 0) {
                        ...
                        listPaySta = paymentStatusController.getListPaymentByCase(objpaySta);
                        loadTablePaymentResult(null);
                        clearForm();
                    }
                    loadComboCriteriaYear();
...
//                        System.out.println("objPaySta.getStmp_report().subSequence(0, 4):" + objPaySta.getStmp_report().subSequence(0, 4));
    ...
    private void loadComboPaymentStatus() {
        /*
         ############ เพิ่ม ขึ้น ตารางเข้า comdobox###############
         */
        int indexTable = 0;
        combo_payment_type.removeAllItems();
        String[] arrTable = ConstantVariable.PROCE_TABLE_PAYMENT;
        for (int i = 0; i < arrTable.length; i++) { //for (String strTable : arrTable) {
            combo_payment_type.addItem(indexTable + " : " + arrTable[i]);
            indexTable++;
        }

        String[] arrTableHC = ConstantVajira.PROCE_TABLE_PAYMENT_HC; // เป็นตารางเพิ่มมาให้เลยแยกกัน
        for (int i = 0; i < arrTableHC.length; i++) { //for (String strTable : arrTable) {
            combo_payment_type.addItem(indexTable + " : " + arrTableHC[i]);
            indexTable++;
        }
        /*
         ############ จบการเพิ่ม ขึ้น ตารางเข้า comdobox###############
         */
