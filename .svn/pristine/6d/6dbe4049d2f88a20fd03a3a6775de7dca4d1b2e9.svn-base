...
import com.claim.dao.VajiraHCDao;
import com.claim.object.HospitalService;
import com.claim.object.ObjRptVajiraHcDetail;
import com.claim.object.ObjRptVajiraHcSummary;
 ...
 * @author POOL_LAPTOP
 */
public class VajiraHCController extends ExcellBaseUtil {
    ...
    static String FILE_NAME_DETAIL = "DetailPay_HC";
    static String FILE_NAME_SUM = "SumPay_HC";

    public static void main(String[] args) {
        VajiraHCController vajira = new VajiraHCController();
...
//        report.setListServiceCode(serviceList);
        vajira.hcHCByHmain(report);
        vajira.hcHcSummary(report);
    }
    ...
    public ProgrameStatus hcHcDetail(OppReport report) {
        ...
        try {
            ...
            //readTemplate 
            file = new FileInputStream(new File("." + File.separator + "xls" + File.separator + "HC_HC_DETAIL.xls"));
            ...
            out = new FileOutputStream(pathDirectory + "" + File.separator + "vajira_HC_" + StringOpUtil.removeNull(report.getServiceCode()) + "_" + report.getStmp() + ".xls");
    ...
    public ProgrameStatus hcHcSummary(OppReport report) {
        ...
        try {
            ...
            out = new FileOutputStream(pathDirectory + "" + File.separator + "vajira_summary_HC_" + report.getYearMonth() + "-" + report.getNo() + ".xls");
    ...
    public ProgrameStatus hcHCByHmain(OppReport report) {
        ProgrameStatus programeStatus = new ProgrameStatus();
        List<HospitalService> listData = new ArrayList<>();
        try {
            connection = new DBManage().open();
            VajiraHCDao vajira11535Dao = new VajiraHCDao();
            ...
            for (int i = 0; i < listData.size(); i++) {
                ...
                programeStatus = hcHcDetail(report);
    ...
    public ProgrameStatus hc_genReportAllFunction(OppReport report) {
        ProgrameStatus programeStatus = new ProgrameStatus();
        try {
            programeStatus = this.hcHCByHmain(report);
            programeStatus = this.hcHcSummary(report);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
