...
public class CreateDataStoreDAO {
    ...
    public ProgrameStatus callOracleStoredGroupReportType1(ObjCallStoreCreateData bean) {
        int status = 0;
    ...
    public ProgrameStatus callOracleStoredGroupReportType2Vajira(ObjCallStoreCreateData bean) {
        int status = 0;
        ProgrameStatus programeStatus = new ProgrameStatus();
        try {

            if (bean.getIndex_table().equals(1)) { // ##################RPT_OPBKK_VAJIRA_HC11535 ########

                //################## ปีงบ 2557 #########
                status = this.callOracleStoredVajira("proc_vajira_hc11535_rpt", bean);
                //################## ปีงบ 2557 #########

            } else if (bean.getIndex_table().equals(2)) { // ##################RPT_OPBKK_VAJIRA_RF ########

                //################## ปีงบ 2557 #########
                status = this.callOracleStoredVajira("proc_vajira_rf_rpt", bean);
                //################## ปีงบ 2557 #########

            } else if (bean.getIndex_table().equals(3)) { // ##################RPT_OPBKK_VAJIRA_HC ########

                //################## ปีงบ 2557 #########
                status = this.callOracleStoredVajira("proc_vajira_clh_rpt", bean);
                //################## ปีงบ 2557 #########
            }
...
//            VSTMP_ROUND := 1;
    ...
    public boolean checkStmpDupicateGroupReportType1(int tableKey, String wordStmp) {
        ...
        try {
            ...
            switch (tableKey) {
                ...
                case 9: // Thaimedicine Mom
                    ...
                    break;
                case 10: // RPT_OPBKK_VAJIRA_HC11535
                    sql.append("SELECT COUNT(*) FROM RPT_OPBKK_VAJIRA_HC11535 WHERE stmp = ?");
                    break;
                case 11: // RPT_OPBKK_VAJIRA_RF
                    sql.append("SELECT COUNT(*) FROM RPT_OPBKK_VAJIRA_RF WHERE stmp = ?");
                    break;
                case 12: // RPT_OPBKK_VAJIRA_HC
                    sql.append("SELECT COUNT(*) FROM RPT_OPBKK_VAJIRA_HC WHERE stmp = ?");
                    break;
                default:
    ...
    public boolean checkStmpDupicateGroupReportType2Vajira(int tableKey, String wordStmp) {
        boolean status = false;
        StringBuilder sql = null;
        ResultSet rs = null;
        PreparedStatement pstmt = null;
        try {
            sql = new StringBuilder();
            switch (tableKey) {
                case 1: // RPT_OPBKK_VAJIRA_HC11535
                    sql.append("SELECT COUNT(*) FROM RPT_OPBKK_VAJIRA_HC11535 WHERE stmp = ?");
                    break;
                case 2: // RPT_OPBKK_VAJIRA_RF
                    sql.append("SELECT COUNT(*) FROM RPT_OPBKK_VAJIRA_RF WHERE stmp = ?");
                    break;
                case 3: // RPT_OPBKK_VAJIRA_HC
                    sql.append("SELECT COUNT(*) FROM RPT_OPBKK_VAJIRA_HC WHERE stmp = ?");
                    break;
                default:
...
//                sb.append(rs.getString("N_RECORD"));
    ...
    private int callOracleStoredVajira(String tableName, ObjCallStoreCreateData bean) {
        StringBuilder storeProce = null;
        CallableStatement callableStatement = null;
        int exec = 0;
        try {
            storeProce = new StringBuilder();
            storeProce.append("{call opbkkclaim.").append(PKG_RPT_58).append(".").append(tableName);
            ...
            callableStatement.setString(7, bean.getStmp_round()); // stmp_round
            callableStatement.setString(8, bean.getStmp_no()); // stmp_no
            callableStatement.setInt(9, bean.getGlobal_budget()); // stmp_no
